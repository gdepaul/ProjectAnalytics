...
public class CICOClient extends JFrame{

	/**
	 * 
	 */
	private static final long serialVersionUID = -5563847155664858821L;
			...
			while(true) {
				try {
					...
					if(obj instanceof Dispatch<?>) { // See if we have a valid command
						Dispatch<CICOClient> command = (Dispatch<CICOClient>)obj;
						command.execute(CICOClient.this);
					}	
				} catch (Exception e) {
					e.printStackTrace();
